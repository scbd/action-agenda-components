<template >
  <multiselect
    ...
    :searchable="true"
    class="fix"
    :placeholder="$t('Type to narrow filter options below or press enter to apply free text search')"
    :selectLabel="$t('Press enter or click to select filter')"
    :tag-placeholder="$t('Press enter to apply free text search or click the Search button')"
    :hide-selected="true"
    ref='multiSelect'
  >
    ...
    <template  slot="placeholder" >
      <Icon name="search" style="max-height: 1em; max-width: 1em;" /> {{$t('Search')}} {{$t('or')}} <Icon name="filter" style="max-height: 1em; max-width: 1em;"/> {{$t('Filter')}}
    </template>

    <template   slot="option" slot-scope="props" >
      <section id="opt-t">
        <div class="container-fluid" v-if="props.option.$groupLabel" style="cursor: pointer;">
          <div class="row" >
            <div class="col-12">
        ...
        <div class="container-fluid" v-if="!props.option.$groupLabel">
          <div  :ref="`${props.option}-opt`" class="row" >
