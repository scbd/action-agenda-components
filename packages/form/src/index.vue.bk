...
<script>
  ...
  export default {
    ...
    components: { FormFeedback, ActorSelect, Action, ActionDetails, Contact, Partners,
                  DebugForm:() => import('./DebugForm') //async load of component ... only if needed
                },
  ...
  function actionComplete(){
    let name        =   this.form.action.name && 
                        this.form.action.name.en

    let description =   this.form.action.description && 
                        this.form.action.description.en

    return name && description
  }

  function updateContacts(){
    if(this.actorType === 'person') this.form.contacts[0] = this.form.actor
  }
