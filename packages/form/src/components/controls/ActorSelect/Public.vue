    ...
    <div class="row">
      <div class="col-12">
        <div class="form-group" :id="`group.${actor.actorType}.name`" >
          <label  :for="`${actor.actorType}.name`"> {{ $t(`Name of the lead public organization`) }} </label>
      ...
      <div class="col-lg-6">
        <div class="form-group" :id="`group.${actor.actorType}.types`" >
          ...
          <SCBDSelect
            ...
            tag-view
            v-validate=""
            :state   ="[ getValidationClass($t(`${actor.actorType}.types`)) ]"
            :name="$t(`${actor.actorType}.types`)"
          />
          <field-error-message :error="errors.collect($t(`${actor.actorType}.types`))" :state="validateState($t(`${actor.actorType}.types`))" />
        </div>
      </div>
      ...
      <div class="col-6" v-if="isOther">
        <div class="form-group" :id="`group.${actor.actorType}.typeOther`" >
          ...
          <input class="form-control"
            ...
            type="text"
            v-model.trim="actor.typeOther[$i18n.locale]"
            v-validate="'required|max:140'"
